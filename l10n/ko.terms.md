...
① 서비스 이용계약은 회원이 본 약관 및 개인정보처리방침 등에 대하여 동의하고 회사가 제공하는 가입양식을 작성하여 서비스 이용을 신청한 경우 회사가 이를 승낙함으로써 이용계약이 체결됩니다.
...
④ 1년이상 허니스크린의 서비스를 사용하지 않은 회원의 경우 허니스크린 계정이 휴면 전환되어 개인정보를 분리 보관합니다.

1. 휴면상태 이전에 보유했던 포인트는 본 약관 제12조 제4항에 정해진 바에 의하여 모두 소멸되며 계정이 복구된 이후에도 복구가 불가합니다.
...
4. 국가비상사태, 서비스 설비의 장애 또는 서비스 이용의 폭주 등 광고서비스 이용에 지장이 있는 때
...
### 제 10 조 (포인트 적립)
① 회사에서 제공하는 광고를 단말기를 통해 회원본인 또는 서포터가 광고를 확인하고 광고에서 요구하는 액션을 수행하면 포인트를 부여 받게 됩니다. 포인트 지급을 위하여 특정 회원의 자격요건이 요구될 수 있습니다.
...
1. 동일한 단말기에서 여러 개의 이메일, 개인정보를 입력하여 2개 이상의 계정을 생성, 이용하는 행위
2. 동일한 사용자가 복수의 계정을 생성한 후 상호 가입자 추천을 하는 행위
3. 타인의 이메일 또는 존재하지 않는 이메일 등을 이용하여 계정을 생성하는 행위
4. 서비스 이용 관련 제반 신청행위 또는 변경행위 시 허위내용 등록행위
5. 서비스 내 게시된 각종 정보의 무단 변경, 삭제 등 훼손 행위
6. 다른 회원의 명예를 손상시키거나 개인정보를 수집하는 행위
7. 회사의 동의 없이 영리 목적의 광고 정보를 전송하거나 회사가 허용한 정보 이외의 다른 정보(컴퓨터 프로그램 및 광고 등)를 송신하거나 게시하는 행위
8. 일체의 가공행위를 통해 서비스를 분해, 모방 또는 변형하는 행위
9. 회사, 기타 제3자의 저작권 등 지적재산권에 대한 침해 행위
10. 외설 또는 폭력적인 정보(메시지, 화상, 음성 등), 기타 공서양속에 반하는 정보를 서비스에 공개하거나 게시하는 행위
...
### 제 21 조 (개인정보보호 의무)
회사는 관련법령이 정하는 바에 따라서 회원 등록정보를 포함한 회원의 개인정보를 보호하기 위하여 노력합니다. 회원의 개인정보보호에 관해서는 관련법령 및 회사가 정하는 개인정보처리방침에서 정한 바에 의하며, 회원이 상시 확인할 수 있도록 홈페이지를 통해 공지하고 있습니다.

### 부칙
본 약관은 2019년 9월 29일부터 적용됩니다.
